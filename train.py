...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    # print("train_path, imgsz, batch_size, gs, opt : ", train_path, imgsz, batch_size, gs, opt)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            pbar = tqdm(pbar, total=nb, leave=True)  # progress bar
        optimizer.zero_grad()
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Forward
            with amp.autocast(enabled=cuda):    # Automatic Mixed Precision (fp16, fp32 mix)
